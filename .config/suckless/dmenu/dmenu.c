...
/* enums */
enum { SchemeNorm, SchemeSel, SchemeOut, SchemeNormHighlight, SchemeSelHighlight, SchemeOutHighlight, SchemeCusSel, SchemeLast }; /* color schemes */
struct item {
	char *text;
...
setup(void)
{
	...
	if (border_width)
		XSetWindowBorder(dpy, win, scheme[SchemeCusSel][ColBg].pixel);
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		/* these options take one argument */
			...
			embed = argv[++i];
		else if (!strcmp(argv[i], "-bc"))  /* custom border color */
			colors[SchemeCusSel][ColBg] = argv[++i];
